...
3.  spans ← apply_context_rules(spans)                     # drop private/DNS IPs and keys < 9 chars or non-gibberish (R8.10 rules); drop example/licence emails (the book's addition)
...
**What the evidence shows.** The memorisation–duplication link is supported by three studies from two overlapping author groups (R8.3 and R8.5 share four authors; R8.6's Wallace and Raffel co-authored R8.4) on different models and corpora and is the best-replicated result in this chapter. Detector quality is single-source per corpus; no cross-corpus comparison of PII recall exists in the opened sources. Residual rates after redaction are reported by none.
...
> **Failure mode — secrets in multi-line or encoded form.** *Symptom:* base64 blobs and PEM bodies pass. *Cause:* line-scoped regex and entropy detectors (R8.22 lists multi-line secrets under "Things That Won't Be Prevented"). *Detection:* PEM headers and long high-entropy blocks in a sample. *Mitigation:* block-level entropy rule; drop the record.
...
For code, the PII types add keys and passwords — R8.10 detected usernames but excluded them from redaction because detection did not improve, and pseudonymised issue authors instead — and the boundary is the file plus the issue thread (R8.10). For dialogue and user interactions ([§11.5](../ch11-synthetic-data-preferences-and-interactive-trajectories/11-5-interactive-collection.md)), names of participants are structural and pseudonymisation must be consistent across turns. For multimodal data, faces, licence plates and text in images require image detectors (Presidio's image-redactor is one interface, R8.21). For agent trajectories, credentials in tool outputs are the dominant secret source and should be scanned at collection time. These are proposals.
