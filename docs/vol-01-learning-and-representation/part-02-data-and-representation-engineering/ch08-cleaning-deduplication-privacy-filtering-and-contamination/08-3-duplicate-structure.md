...
Complexity: O(b·N) insertions; candidate pairs O(Σ_buckets |B|²) worst case, O(N) when b, k and document length are all ≪ N (R8.3 App. A). Memory: one table per band; datatrove writes per-bucket signature files and sorts them so that step 2 is a merge over sorted runs with `world_size` divisible by `num_buckets` (R8.16 `MinhashDedupBuckets`, OFFICIAL-DOCUMENTATION).
...
```text
Algorithm 8.7 — Suffix-array exact-substring deduplication (R8.3; mask and drop actions from P04)
...
- **Expected result.** A > B, as P06 reports (P06 gives no seed spread, so "beyond noise" is part of what this experiment measures); C ≈ A if the multiplicity hypothesis holds.
